...
let
  ...
  sourceMapOf = h:
    if !(builtins.isAttrs h) || !(h ? _htag) then SM.leaf h
      ...
      # and surfacing `DConLayer:0 .. DConLayer:n` alongside the raw
      # operand positions `MuDesc | MuIndex | MuPayload`.
      #
      # Per-layer slot map:
      ...
      # `MuPayload`.
      #
      # Non-trampoline shape (top `peelLayer` returns null): emit only
      # the top-level Mu slots, with no DConLayer enumeration.
        let
          isRetLeaf = p:
        ...
        if topPeel == null then
          ...
          let
            ...
            layerKeys = builtins.listToAttrs (builtins.genList
              (k: {
    ...
    in
    {
      ...
      "indexed-pi-desc-DPiFn-is-branch-function" = {
        expr =
          ...
          in
          (SMf.descendChain [ P.AnnTerm P.DPiFn ] sm).hoas._htag;
        expected = "ann";
      };
      ...
      "allD-DElimLevel-maps-to-outer-level" = {
        expr =
          ...
          in
          (SMf.descendChain [ P.DElimLevel ] sm).hoas._htag;
        expected = "level-zero";
      };
      "allD-Scrut-is-d" = {
        expr =
          let
            sm = sourceMapOf
              (H.allD H.levelZero H.unit H.string H.levelZero H.int_ H.float_ H.tt H.attrsLit);
          in
          (SMf.descendChain [ P.Scrut ] sm).hoas._htag;
        expected = "attrs-lit";
      };
          ...
          let
            sm = sourceMapOf
              (H.descElim H.unit H.levelZero H.levelZero
                ...
                H.tt);
          in
          (SMf.descendChain [ P.AnnType ] sm).hoas._htag;
        expected = "unit";
      };
          ...
          let
            sm = sourceMapOf
              (H.descElim H.unit H.levelZero H.levelZero
                ...
                H.tt);
          in
          (SMf.descendChain [ (P.Case "onArg") ] sm).hoas._htag;
        expected = "float";
      };
          ...
          let
            sm = sourceMapOf
              (H.descElim H.unit H.levelZero H.levelZero
                ...
                H.tt);
          in
          (SMf.descendChain [ P.Motive ] sm).hoas._htag;
        expected = "string";
      };
      ...
      "descCon-nontrampoline-MuPayload-resolves" = {
        expr =
          let
            standalone = H.descCon H.unit H.int_ H.bootRefl;
            sm = sourceMapOf standalone;
          in
          (SMf.descendChain [ P.MuPayload ] sm).hoas._htag;
        expected = "boot-refl";
      };
      "descCon-trampoline-natChain-DConLayer-0-MuIndex" = {
        expr =
          ...
          in
          (SMf.descendChain [ (P.DConLayer 0) P.MuIndex ] sm).hoas._htag;
        expected = "string";
      };
      "descCon-trampoline-natChain-DConLayer-1-MuIndex" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          (SMf.descendChain [ (P.DConLayer 1) P.MuIndex ] sm).hoas._htag;
        expected = "path";
      };
      "descCon-trampoline-natChain-DConLayer-base-MuIndex" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          (SMf.descendChain [ (P.DConLayer 2) P.MuIndex ] sm).hoas._htag;
        expected = "attrs";
      };
      "descCon-trampoline-natChain-DConLayer-base-MuPayload" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          (SMf.descendChain [ (P.DConLayer 2) P.MuPayload ] sm).hoas._htag;
        expected = "boot-inl";
      };
      "descCon-trampoline-top-level-MuPayload-resolves" = {
        expr =
          let
            D_ = H.unit;
            base = H.descCon D_ H.attrs (H.bootInl D_ D_ H.bootRefl);
      ...
      "descCon-trampoline-natChain-nonbase-has-no-MuPayload" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          SMf.descendChain [ (P.DConLayer 0) P.MuPayload ] sm;
        expected = null;
      };
      "descCon-trampoline-natChain-out-of-range-is-null" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          SMf.descendChain [ (P.DConLayer 99999) P.MuIndex ] sm;
        expected = null;
      };
      "descCon-trampoline-consChain-DConLayer-0-Elem-0" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          (SMf.descendChain [ (P.DConLayer 0) (P.Elem 0) ] sm).hoas._htag;
        expected = "string";
      };
      "descCon-trampoline-consChain-DConLayer-1-Elem-0" = {
        expr =
          let
            ...
            sm = sourceMapOf layer0;
          in
          (SMf.descendChain [ (P.DConLayer 1) (P.Elem 0) ] sm).hoas._htag;
        expected = "attrs";
      };
      ...
      # -- Stack-safety + DConLayer quotient at 5000 depth. A kernel-
      #    emitted `[DConLayer 4000, Elem 0]` chain resolves through
      #    the walker to the layer-4000 head HOAS. --
