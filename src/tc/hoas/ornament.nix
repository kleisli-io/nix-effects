...
let
  ...
  functionalLawDiagnosticRecords = F:
    ...
    in builtins.concatLists (builtins.attrValues (builtins.mapAttrs checkOne checks));
  ...
  functionalCompose = outer: inner:
    let
      ...
      section = i: baseValue:
        ...
        in fOuter.section mid innerValue;
      chooseIndex = i: baseValue:
        let
          mid = fInner.chooseIndex i baseValue;
          innerValue = fInner.section i baseValue;
        in fOuter.chooseIndex mid innerValue;
    in functionalOrnament {
    ...
    let
      ...
      baseInput =
        self.app
          ...
          ornamentedInput;
    in ornBuild output outputIndex (self.app args.fn baseInput);
    ...
    in
      if D._htag == "ann" then algShapeDiagnosticRecordsAt path D.term alg
        ...
        if node == null || node.tag != "arg" then [ (algMismatchDiagnostic path "arg" D got) ]
          ...
          else algShapeDiagnosticRecordsAt "${path}.arg"
            (D.body self.ttPrim)
            sampled.value
      else if D._htag == "desc-rec-enc" then
        if node == null || node.tag != "rec" then [ (algMismatchDiagnostic path "rec" D got) ]
          ...
          let sampled = sampleAlgBodyResult path node.body; in
          if !sampled.ok then sampled.diagnostics
          else algShapeDiagnosticRecordsAt "${path}.rec"
            D.D
            sampled.value
      else if D._htag == "desc-plus-enc" then
  ...
  datatypeFieldFromSpec = cfg: baseFields: spec:
    if fieldSpecKind spec == "insert" then mkInsertedField spec
    else
      let
      ...
      if base.kind == "data" then (mkDataField base) // fieldMetadata spec
        ...
        (if isUnitTy cfg.J then self.piFieldAt (base.level or 0) base.name base.S
         else self.piFieldAtIndex (base.level or 0) base.name base.S
        ...
        (if isUnitTy cfg.J then self.piFieldDAt (base.level or 0) base.name base.SFn
         else self.piFieldDAtIndex (base.level or 0) base.name base.SFn
           (prev: s: piBranchIndexFor cfg base spec prev s)) // meta
  ...
  ornamentDatatype = base: spec:
    if !(builtins.isAttrs base && base ? _cons && base ? T && base ? D) then
      ...
      in builtins.seq _validation (dt // {
        ornament = core;
        inherit forget;
        _ornMeta = {
...
in {
  scope = {
    ...
    ornMu = ornMu;
    ornForget = ornForget;
    ornPullback = ornPullback;
    ornLiftFold = ornLiftFold;
    ornLiftProducer = ornLiftProducer;
    ornLiftTransform = ornLiftTransform;
    functionalOrnament = functionalOrnament;
    functionalOrnamentDiagnosticRecords = functionalOrnamentDiagnosticRecords;
    functionalOrnamentDiagnostics = functionalOrnamentDiagnostics;
    validateFunctionalOrnament = validateFunctionalOrnament;
    tryFunctionalOrnament = tryFunctionalOrnament;
    ornSection = ornSection;
    ornTargetIndex = ornTargetIndex;
    ornIndexProof = ornIndexProof;
    ornBuild = ornBuild;
    functionalLawDiagnosticRecords = functionalLawDiagnosticRecords;
    functionalLawDiagnostics = functionalLawDiagnostics;
    validateFunctionalLaws = validateFunctionalLaws;
    checkFunctionalLaws = checkFunctionalLaws;
    functionalCompose = functionalCompose;

    ornRet = j: proof: baseIndex:
    ...
    algRet = result: mkAlg "ret" { inherit result; };
    algArg = body: mkAlg "arg" { inherit body; };
    algRec = body: mkAlg "rec" { inherit body; };
    algPiKeep = branch: body:
      mkAlg "pi" ((if builtins.isAttrs branch then branch else { branchIndex = branch; }) // { inherit body; });
    algPlus = left: right: mkAlg "plus" { inherit left right; };
    algOrn = algOrn;
    algOrnDiagnosticRecords = algOrnDiagnosticRecords;
    algOrnDiagnostics = algOrnDiagnostics;
    validateAlgOrn = validateAlgOrn;
    tryAlgOrn = tryAlgOrn;

    ornId = ornId;
    ornCompose = compose;
    ornament = ornamentDatatype;
    ornamentDiagnosticRecords = ornamentDiagnosticRecords;
    ornamentDiagnostics = ornamentDiagnostics;
    validateOrnament = validateOrnament;
    tryOrnament = tryOrnament;
  };
  ...
  tests =
    let
      ...
      listNatToVecOrn =
        ...
        in mk listNatD (H.ornPlus nilBranch consBranch);
      listNatLengthAlg =
        H.algPlus
          (H.algRet H.zero)
          (H.algArg (_x:
    ...
    in {
      "orn-id-desc-ret-shape" = {
        expr = Q.nf [] (H.elab (H.ornDesc (H.ornId H.unitPrim H.descRet)))
            == Q.nf [] (H.elab H.descRet);
        expected = true;
      };
      ...
      "orn-id-forget-ret-checks" = {
        ...
        in (H.checkHoas T (H.app (H.app (H.ornForget O) H.ttPrim) x)).tag;
        expected = "app";
      };
      ...
      "orn-law-id-forget" = {
        ...
        in Q.nf [] (H.elab lhs) == Q.nf [] (H.elab x);
        expected = true;
      };
      ...
      "orn-ret-forget-uses-desc-leaf-eq-orientation" = {
        ...
        in !((H.checkHoas forgetTy (H.ornForget O)) ? error);
        expected = true;
      };
      ...
      "orn-insert-desc-shape" = {
        expr =
          Q.nf [] (H.elab (H.ornDesc insertBool))
          == Q.nf [] (H.elab (H.descArg H.unitPrim 0 H.bool (_: baseD)));
        expected = true;
      };
      ...
      "orn-rec-forget-recurses" = {
        ...
        in (H.checkHoas T (H.app (H.app (H.ornForget O) H.ttPrim) succZero)).tag;
        expected = "app";
      };

      "orn-id-desc-pi-shape" = {
        expr = Q.nf [] (H.elab (H.ornDesc (H.ornId H.unitPrim piVoidD)))
            == Q.nf [] (H.elab piVoidD);
        expected = true;
      };
      ...
      "ornPullback-matches-forget-then-run" = {
        ...
        in Q.nf [] (H.elab lhs) == Q.nf [] (H.elab rhs);
        expected = true;
      };
      ...
      "ornLiftFold-list-length-checks" = {
        ...
        in if r ? error then r.msg else r.tag;
        expected = "app";
      };
      ...
      "functionalOrnament-build-section-forget-law" = {
        ...
        in Q.nf [] (H.elab forgot) == Q.nf [] (H.elab baseValue);
        expected = true;
      };
      ...
      "orn-law-alg-index-agrees-with-fold" = {
        ...
        in Q.nf [] (H.elab folded) == Q.nf [] (H.elab (H.succ H.zero));
        expected = true;
      };
      ...
      "orn-compose-forget-drops-in-order" = {
        ...
        in if r ? error then r else r.tag;
        expected = "app";
      };
      ...
      "ornament-sugar-inserted-field-forget-checks" = {
        ...
        in (H.checkHoas Box.T (H.app TaggedBox.forget0 tagged)).tag;
        expected = "app";
      };

      "orn-law-sugar-core-agreement" = {
        expr =
          Q.nf [] (H.elab (H.ornDesc TaggedBox.ornament))
          == Q.nf [] (H.elab (H.ornDesc TaggedBoxCoreOrn));
        expected = true;
      };
      ...
      "ornament-sugar-indexed-forget-checks" = {
        ...
        in !((H.checkHoas forgetTy VecFromListSugar.forget) ? error);
        expected = true;
      };
      ...
      "orn-regression-sugar-reindexed-rec-forget-applies" = {
        ...
        in (H.checkHoas target forgot).tag;
        expected = "app";
      };
        ...
        expr = let
          ...
          forgetTy =
            H.forall "b" H.bool (b:
              H.forall "_" (H.muI H.bool D b) (_:
                H.muI H.bool IndexedPiBase.D b));
        in !((H.checkHoas forgetTy IndexedPiKept.forget) ? error);
        expected = true;
      };
      ...
      "ornDesc-invalid-node-still-throws" = {
        ...
        in (builtins.tryEval (builtins.deepSeq (H.elab (H.ornDesc bad)) true)).success;
        expected = false;
      };
