...
let
  ...
  normalizeMeta = meta:
    let
      rawConstructors =
        if meta ? constructors then meta.constructors
        else if meta ? cons then meta.cons
        else [];
      constructors = map normalizeConstructor rawConstructors;
    in meta // {
...
in {
  scope = {
    ...
    normalizeMetadata = normalizeMeta;

    datatypeInfo = infoOf;

    instantiate = spec: paramArgs:
    ...
    fields = con: con.fields or [];

    constructorByName = info: name:
    ...
    fieldRole = field: field.role or null;
  };
  ...
  __docs = {
    ...
    normalizeMetadata = {
      description = "normalizeMetadata: canonicalise a raw `_dtypeMeta` attrset — defaults `indexed`/`params`/`paramArgs`/`indexTy`, normalises both `constructors` and the legacy `cons` alias, and pre-fills every field with `level`/`eq`/`proof`/`type`/`typeFn`/`S`/`SFn`/`idxFn`/`branchIdxFn`/`role`/`source` slots.";
