...
rec {
  __example = {
    title = "Verified Functions";
    description = "Kernel-checked HOAS programs extracted into plain Nix functions.";
    introduction = ''
    ...
    sections = [
      {
        ...
        prose = ''
          The smallest verified programs are ordinary unary functions. The
          type says what the extracted Nix function accepts and returns; the
          implementation is written with verified HOAS constructors.
        '';
        code = ''
      ...
      }
      {
        ...
        prose = ''
          List combinators remain checked HOAS terms until extraction. The
          composed example filters zeros out of a list and folds the remaining
          values into a sum.
        '';
        code = ''
      ...
      {
        title = "Sums, lets, pairs, and records";
        prose = ''
          Verified extraction also covers the richer HOAS constructors used by
          nix-effects types: coproduct elimination, local bindings, sigma
          pairs, and record field projection.
        '';
        code = ''
      ...
      {
        title = "Strings and records";
        prose = ''
          The final examples use kernel-verified string predicates directly
          and then combine them with record projection.
        '';
        code = ''
  ...
  filterZerosResult = v.verify (H.listOf H.nat)
    (v.filter H.nat
      ...
      (H.cons (v.nat 0) (H.cons (v.nat 1)
        (H.cons (v.nat 0) (H.cons (v.nat 2) H.nil)))));
  ...
  foldSumResult = v.verify H.nat
    (v.fold H.nat H.nat (v.nat 0)
      ...
      (H.cons (v.nat 1) (H.cons (v.nat 2)
        (H.cons (v.nat 3) H.nil))));
  ...
  composedResult =
    ...
    in
    v.verify H.nat (v.fold H.nat H.nat (v.nat 0) addCombine filtered);
