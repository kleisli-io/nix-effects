...
# pre-existing Σ / + / Π / Unit / Empty / mu reductions.
#
...
{ self, api, ... }:
{
  scope = {
    True = api.leaf {
      description = "True: propositional truth — alias for `unit` per HoTT Book, section 1.7 (`True = ⊤`). Distinct from `BoolDT`'s data-level `true_`.";
      signature = "True : Hoas";
      value = self.unit;
    };
    False = api.leaf {
      description = "False: propositional falsehood — alias for the kernel-primitive `void` (`empty`) per HoTT Book, section 1.7 (`False = ⊥`); the initial-object dual of `True`.";
      signature = "False : Hoas";
      value = self.void;
    };
    and = api.leaf {
      description = "and: propositional conjunction — `and P Q := Σ (_:P). Q` per HoTT Book, section 1.7. Elaborates to a sigma.";
      signature = "and : Hoas -> Hoas -> Hoas";
      value = P: Q: self.sigma "_" P (_: Q);
    };
    or_ = api.leaf {
      description = "or_: propositional disjunction — `or_ P Q := P + Q` per HoTT Book, section 1.7. Trailing underscore avoids the Nix `or` keyword in identifier position (mirrors `true_` / `false_`).";
      signature = "or_ : Hoas -> Hoas -> Hoas";
      value = P: Q: self.sum P Q;
    };
    not = api.leaf {
      description = "not: propositional negation — `not P := P → ⊥` per HoTT Book, section 1.7. Elaborates to a pi with `void` codomain.";
      signature = "not : Hoas -> Hoas";
      value = P: self.forall "_" P (_: self.void);
    };
    iff = api.leaf {
      description = "iff: propositional biconditional — `iff P Q := (P → Q) × (Q → P)` per HoTT Book, section 1.7. Built as `and (P→Q) (Q→P)`.";
      signature = "iff : Hoas -> Hoas -> Hoas";
      value = P: Q: self.and (self.forall "_" P (_: Q))
        (self.forall "_" Q (_: P));
    };
    ...
    dec = api.leaf {
      ...
      value = P: self.sum P (self.not P);
    };
    yes = api.leaf {
      description = "yes: positive decidability witness — `yes P p : dec P` for a proof `p : P`. Routes through the left injection of `P ⊎ ¬ P`.";
      signature = "yes : Hoas -> Hoas -> Hoas  -- P, proof";
      value = P: p: self.inlAtExplicit 0 P (self.not P) p;
    };
    no = api.leaf {
      description = "no: negative decidability witness — `no P r : dec P` for a refutation `r : not P`. Routes through the right injection of `P ⊎ ¬ P`.";
      signature = "no : Hoas -> Hoas -> Hoas  -- P, refutation";
      value = P: r: self.inrAtExplicit 0 P (self.not P) r;
    };
    decElim = api.leaf {
    ...
    decOr = api.leaf {
      ...
      value = P: Q: dp: dq:
        let
          inherit (self) lam app inlAtExplicit inrAtExplicit sumElim;
          result = self.or_ P Q;
        in
        self.decElim P
          (lam "_" (self.dec P) (_: self.dec result))
          (lam "p" P (p: self.yes result (inlAtExplicit 0 P Q p)))
          (lam "np" (self.not P) (np:
            self.decElim Q
              (lam "_" (self.dec Q) (_: self.dec result))
              (lam "q" Q (q: self.yes result (inrAtExplicit 0 P Q q)))
              (lam "nq" (self.not Q) (nq:
                self.no result
                  (lam "pOq" result (pOq:
                    sumElim 0 P Q
    ...
    leRefutSuccZero = api.leaf {
      ...
      value = m: pf:
        ...
        in
        leElim 0 K Pz Ps (succ m) zero pf;
    };
    ...
    leInjSS = api.leaf {
      ...
      value = m: n: pf:
        ...
        let
          ...
          Pz = lam "n" nat (_: leZ);
          Ps = lam "m'" nat (mp: lam "n'" nat (np:
            lam "lemn" (le mp np) (lemn:
              lam "_ih" (le (app self.predNat mp) (app self.predNat np)) (_:
                lemn))));
        in
        leElim 0 K Pz Ps (succ m) (succ n) pf;
    };
          ...
          lam "ihM" (forall "_n" nat (n': decAt mp n')) (ihM:
            lam "n" nat (nv:
              ind 0
                ...
                nv)));
      in
      lam "m" nat (mv: ind 0 outerMot baseM stepM mv);
    decideEqNatTm = self.elab self.decideEqNatHoas;
    decideEqNat = api.leaf {
      ...
      value = { _htag = "pre-elab"; tm = self.decideEqNatTm; };
    };
    ...
    decideLeNatHoas =
      let
        ...
        baseM = lam "n" nat (nv: yes (le zero nv) leZ);
    ...
    decideLeNat = api.leaf {
      ...
      value = { _htag = "pre-elab"; tm = self.decideLeNatTm; };
    };
    ...
    signsDiffer = api.leaf {
      description = "signsDiffer: no-confusion refutation `(m n : Nat) -> Eq IntZ (pos m) (negSucc n) -> void`. McBride 2000 PhD, section 3.5 discriminator-motive technique applied to the IntZ sign discriminator (`unit` on `pos`, `void` on `negSucc`) plus `bootJ` transport.";
    ...
    intzLe = api.leaf {
      ...
      value = m: n:
        let
          inherit (self) lam nat intz intzElim le unit void u;
        in
        intzElim 1
          (lam "_z" intz (_: u 0))
          (lam "mp" nat (mp:
    ...
    decideEqIntZ = api.leaf {
      ...
      value = { _htag = "pre-elab"; tm = self.decideEqIntZTm; };
    };
    ...
    decideLeIntZ = api.leaf {
      ...
      value = { _htag = "pre-elab"; tm = self.decideLeIntZTm; };
    };
