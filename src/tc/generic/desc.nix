...
let
  ...
  reconstruct = view: children:
    ...
    else withReconstructedLabels view (
      if view.idx == 0 then
        vAppN encRetVal [ view.I view.k view.j ]
      else if view.idx == 1 then
        if isHomogeneous view.l view.k
        then vAppN encArgVal
               [ view.I view.k view.sTyRaw children.tFn ]
        else vAppN encArgAtVal
               [ view.I view.k view.l view.eq view.sTyRaw children.tFn ]
      else if view.idx == 2 then
        vAppN encRecVal [ view.I view.k view.j children.sub ]
      else if view.idx == 3 then
        if isHomogeneous view.l view.k
        then vAppN encPiVal
               [ view.I view.k view.sTyRaw children.fn children.sub ]
        else vAppN encPiAtVal
               [ view.I view.k view.l view.eq view.sTyRaw
                 children.fn children.sub ]
      else if view.idx == 4 then
        vAppN encPlusVal [ view.I view.k children.A children.B ]
  ...
  foldDescM = handlers: d:
    ...
    in
      if view.idx == 0 then
        ...
        K.bind (recur (applyDescFn view.tFn placeholder)) (sampleR:
          (handlerOr handlers "arg" default) {
            inherit view;
            inherit (view) sTy;
            body = arg: recur (applyDescFn view.tFn arg);
  ...
  foldDescWithPath = path: handlers: d:
    ...
    in
      if view.idx == 0 then
        ...
        bindP P.DArgBody (recurAt P.DArgBody (applyDescFn view.tFn placeholder)) (sampleR:
          (handlerOr handlers "arg" default) {
            inherit view path;
            inherit (view) sTy;
        ...
        bindP P.DRecTail (recurAt P.DRecTail view.sub) (subR:
          (handlerOr handlers "rec" default) {
            inherit view path;
            inherit (view) j;
            sub = subR;
        ...
        bindP P.DPiBody (recurAt P.DPiBody view.sub) (subR:
          (handlerOr handlers "pi" default) {
            inherit view path;
            inherit (view) sTy fn;
            sub = subR;
...
in {
  scope = {
    ...
    foldDescWithPath = foldDescWithPath;
    ...
    isRet = d: (rawView d).idx == 0;
    isArg = d: (rawView d).idx == 1;
    isRec = d: (rawView d).idx == 2;
    isPi = d: (rawView d).idx == 3;
    isPlus = d: (rawView d).idx == 4;
  };
  ...
  tests = {
    ...
    "desc-view-pi-with-label" = {
      ...
      in (self.descView (H.withDescLabel "f" Dpi)).label;
      expected = "f";
    };
    ...
    "desc-deepEqualDesc-labels-irrelevant" = {
      ...
      in self.deepEqualDesc labeled plain;
      expected = true;
    };
    ...
    "desc-mapDesc-identity-preserves-label" = {
      ...
      expr = let
        D = H.withDescLabel "x"
              (H.descArg H.unit 0 H.nat (_: H.descRet));
      in (self.descView (self.mapDesc (x: x.default) D)).label;
      expected = "x";
    };
    ...
    "desc-mapDesc-identity-preserves-conLabel" = {
      expr = let
        D = H.withConLabel "mk"
              (H.descArg H.unit 0 H.nat (_: H.descRet));
      in (self.descView (self.mapDesc (x: x.default) D)).conLabel;
      expected = "mk";
    };
    ...
    "datatype-label-irrelevance-preserved-after-wiring" = {
      ...
      in self.deepEqualDesc T1.D T2.D;
      expected = true;
    };
    ...
    "desc-map-preserves-shape" = {
      expr = let D = H.plus H.descRet (H.descRec H.descRet);
    ...
    "desc-mapDesc-non-trivial-replacement" = {
      ...
      in self.shape (self.mapDesc swap D);
      expected = {
        kind = "plus";
    ...
    "desc-mapDesc-arg-identity" = {
      ...
      expr = let Darg = H.descArg H.unit 0 H.nat (_: H.descRet);
    ...
    "desc-foldDescWithPath-bodyPath-helper-matches-descent" = {
      ...
      expr = let
        Darg = H.descArg H.unit 0 H.nat (_: H.descRet);
      in (self.foldDescWithPath [ P.MuPayload ] {
        ret = x: K.pure null;
    ...
    "desc-foldDescWithPath-nested-descent-chain-outermost-first" = {
      ...
      in { outer = outer.position; inner = inner.position; };
      expected = { outer = P.DPlusL; inner = P.DRecTail; };
    };

    "desc-foldDescWithPath-leaf-typeError-preserved-under-chain" = {
      # The original kernel error is preserved at the bottom of the
      # nested-edges chain — wrapping only adds blame, never replaces
      # the leaf. Equality is checked inside `expr` so the kernel-
      # error fixture stays in the same scope.
      expr = let
        Derr = fx.diag.error.mkKernelError {
          rule = "foldDescWithPath";
          msg = "leaf preserved";
      ...
      in walkToLeaf err.__surfacedError == Derr;
      expected = true;
    };
