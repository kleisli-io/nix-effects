        ...
        # decomposition.
          ...
          if tagOfDesc D != "DViewPlus" then fallback
            ...
            then rawList (self.reifyType (argSOf B))
              ...
              && (
              let
                bA = applyArgT A V.vTt;
                bB = applyArgT B V.vTt;
              in
      ...
      value = hoasTy: tyVal: val0:
        ...
        # -- Compound types (record, maybe, variant) --
          ...
          if n == 0 then { }
            ...
            in
            extractFields fields tyVal val
          ...
          in
          extractBranch branches tyVal val
        ...
        # No kernel extraction needed — the function was carried opaquely.
          ...
          # supporting both dependent and non-dependent Pi.
            ...
            in
            self.extractInner codomainHoas codomainTyVal resultVal
        ...
        # "_field0").
          ...
          let
            ...
            isSumPlusDesc = d:
              ...
              && (
                let
                  bA = applyArgT (plusAOf d) V.vTt;
                  bB = applyArgT (plusBOf d) V.vTt;
                in
                tagOfDesc bA == "DViewRet" && tagOfDesc bB == "DViewRet"
              );
          ...
          then self.extractInner { _htag = "nat"; } tyVal val
            ...
            let
              leftTyVal = argSOf (plusAOf descTyVal);
    ...
    in
    {
      ...
      "reifyType-encoded-non-prelude-mu-fallback" = {
        expr =
          ...
          in
          "${encodedNonPreludeDesc.tag}/${h._htag}/${h.D._htag}";
