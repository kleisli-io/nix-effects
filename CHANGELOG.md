...
- **Kernel evaluation enters through a depth-budgeted hybrid front end.** `evalF` routes through `src/tc/eval/direct.nix`: shallow structural terms run direct `mkValueF` recursion under a depth budget threaded through sub-evaluations, bottoming out in the machine at budget exhaustion, so libnix stack depth stays O(budget) at any term depth. The description-eliminator family (`desc-ind`, `interp-d`, `all-d`, `everywhere-d`) and desc-con chain construction always evaluate through the machine, preserving the machine-internal value invariants and the single cert-construction path.
...
- **The bench gate verdict is driven by representation-invariant axes; per-counter alloc deltas are demoted to diagnostic blame.** The old hard-fail rule — max ‰ delta across per-category alloc counters — is representation-dependent: a faithful representation change relocates allocation between categories (one counter down, another up) and read as a large regression despite near-flat total heap bytes. The verdict is now the Σ-bytes aggregate (values+envs+sets+list `.bytes`, code-size excluded; tolerance 50‰) plus the step axis below. The per-counter max survives as a tight safety-net behind an absolute floor (`allocAbsoluteFloor`): a counter drives a fail only when it moves both relatively (>5‰) and absolutely (≥ floor), so small-base artifacts cannot set the verdict. Failing workloads render a signed blame section (risers and drops, sorted by |‰|) so counter relocation reads at a glance. Re-gating recorded history: 12 relocation-style false fails now pass, 21 genuine Σ-bytes regressions still fail, step axis Δ0‰. 30 nix-unit fixtures pin classify, the floor, both axes, and the markdown serialiser.
...
- **CEK step-count work axis in the bench gate.** A parallel counted machine driver (`runMachineCountedF`; production driver untouched, measured zero per-step cost) reports the exact CEK transition count of normalising depth-scaling probe terms at two depths; the per-level slope is checked against a declared contract (`budgets.toml` `[stepSlopes]`, tolerance 5‰). The count is deterministic — zero measurement noise — and invariant across faithful representation changes, so any slope move is a real work regression. Probes `tc.vAppF` (slope 4) and `tc.descInd` (slope 193) share term builders with the alloc workloads so both axes gate identical terms; slope values are generated by `bench/runner/step-budgets.nix` and spliced, never hand-typed.
- **Counted conversion probes on the step axis.** `runConvF` is round-based — a depth-n spine peels flat in one round, so round count carries no work signal. `runConvCountedF` (sibling entry sharing the peel arms; production path untouched) counts dispatched conversion goals: steps = Σ |goals| per BFS round. Probes gate the two structural peel arms on value chains: `tc.convPair` (slope 1) and `tc.convInl` (slope 2); an extra sibling obligation per layer moves the slope 1→2. `VDescCon` chains are not peel arms (one base goal regardless of depth), so the probes use structural value chains — that coverage boundary is documented in `bench/step-probes.nix`.
- **Peak-heap soft-warn axis.** The evaluator's `gc.heapSize` (≈ peak committed heap) is threaded through the run summary instead of dropped, and the gate reports a rise above `peakHeapWarnPct` (25%) as an advisory warning. Peak live heap is the true space signal but GC-timing-nondeterministic, so it never contributes to a hard fail.
- **The bench runner pins `GC_INITIAL_HEAP_SIZE=8M` on measured invocations.** `gc.heapSize` never reads below the GC's starting heap, so an ambient default (~403 MB observed) floor-clamps peak-heap for every workload under it — the soft-warn axis was comparing floors, not peaks. The pin is alloc-neutral (counters byte-identical with/without) and step counts are untouched; cpuTime rises ~20% on heap-heavy workloads from extra GC cycles, so cpu numbers recorded under an ambient heap are not comparable. The run envelope and history JSON record `gcInitialHeapSize` for provenance.
...
- **Deferred desc-ind motives force before their domain is read.** A computed motive (an application, projection, or let-bound thunk) arrives as a deferred term record; consumers demanding the elimination's index type — quoting a stuck `everywhere-d` spine, conversion touching an induction-hypothesis binder domain — threw a spurious `requires VLam motive (got VThunkTm)` on legal programs. The motive is now forced at the point of demand inside the lazy index-type binding (memoized, one shared machine run); desc-ind-heavy allocation counters stay flat to ±1‰.
...
- **Documentation: the 0.12.0 "Handler-state transport via Thunk" note misattributed the `deepSeq` hazard.** That entry said a real derivation's cyclic `passthru`/`all`/`outputs` graph "hangs `builtins.deepSeq`," implying `deepSeq` has no cycle detection. It does: `forceValueDeep` carries an identity seen-set, so a self-referential attrset terminates — verified identical across Nix 2.3.18, 2.18.8, 2.24.8, Lix 2.91.1, and 2.35pre. The carrier is still required, for accurate reasons: identity-based detection is defeated by lazy graphs that regenerate a fresh object on each force (uncatchable overflow); deep-forcing a real derivation's full attribute closure at every trampoline step is prohibitively expensive; and the guard-less `api.extractValue` walk would not terminate on a raw cyclic value. The original "uncatchable by `tryEval`" point stands — a stack overflow and `toJSON`'s function-conversion error both escape `tryEval`. Rationale corrected in `book/src/trampoline.md` § "State-shape contract"; pinned by `deepSeq` cycle and throw tests in `src/state/thunk.nix`.

### Performance

Allocation counters below are evaluator-deterministic (measured under the pinned Nix). CPU timings are single-host wall-clock samples — non-deterministic and hardware-dependent; treat them as rough indications only.

- **Known regression: the depth-flat machinery costs CPU and allocations.** Measured with the bench suite at the flake-pinned evaluator (no Nix-version confound), the full typechecker test suite costs ~4× CPU (29s → 119s) and 2.4–4.4× allocations versus 0.12.0; kernel workload medians regress across evaluator counters (attribute lookups +83%, function calls +50%, environments +46%, attrsets +40%, thunks +27%). The cost concentrates where blame brackets and trampoline churn are dense (worst workloads ~11×), not where terms are deep (pure deep chains ~+16% CPU); the `effects.*` layer is unaffected. This is the current price of O(1) host call-stack depth; attribution and optimization are in progress — the entries below record the recovery landed so far.
- **`bindP` chain positions force only on the error path.** An eager empty-list test forced the accumulated position list on every call, paying O(N²) list elements on all-green N-deep checks for a value only the error path reads. `tc.checkD.mixed-chain-5000`: values 13.93M → 1.40M, listElements 25.2M → 0.18M, CPU 0.94 s → 0.55 s. Error traces unchanged.
- **Hybrid front end takes shallow evaluation off the machine.** The machine's per-step ceremony dominated checker-side cost on small shallow eval entries; the depth-budgeted direct path removes it. `tc.checkD.mixed-chain-5000`: lookups 1.29M → 0.69M, CPU 0.55 s → 0.30 s; `tc.ornaments.alg-list-to-vec-check`: CPU 1.51 s → 0.82 s.
- **Non-structural conversion goals skip the per-goal machine setup.** The conv knot classifies each goal once: structural-spine and binder goals stay on the flat goal stack, base goals dispatch `convStep` directly instead of paying a fresh BFS setup. `tc.bindP.desc-con-trampoline-blame-5000`: values 39.6M → 36.3M, CPU 13.9 s → 12.8 s.
- **Desc-con chain layers are attested individually.** One odd element no longer drops the whole chain onto the per-layer slow path: the elaborator attests each layer whose field arguments match the validated representative on its own certificate, and the kernel checker consults the layer's certificate instead of the outermost one. `tc.bindP.desc-con-trampoline-blame-5000`: values 36.3M → 2.5M, lookups 55.3M → 2.0M, CPU 12.8 s → 0.9 s.
- **Constant chunk ladder for the machine driver.** Per-run chunk doubling is replaced by a linked ladder of shared constant index lists (4 … 32768): shallow machine entries stop early and no chunk list is allocated per run. Deferred-term records also memoize their forced value, so repeated forces share one machine run. 36-workload sweep: 125 counters improved — `tc.decidable.leDiag50` lookups −303‰, `experimental.descInterp.stateChain.s1k` lookups −262‰ and CPU 7.39 s → 6.36 s.
- **Machine slimmed to live transitions only.** Never-pushed frame constructors and an unreachable desc-ind linear-peel branch are removed after trace instrumentation measured them at zero firings suite-wide; `deadnix` now reports zero unused bindings in `machine.nix`. Dropping the branch's per-entry `let` thunks measured −1.2‰ thunks on desc-interpreter workloads.
- **Remaining gap after the recovery work above.** Kernel-heavy quick-tier workloads still allocate above the pre-rework baseline (`tc.decidable.leDiag50` values ×3.1, `experimental.descInterp.stateChain.s1k` ×3.0, `tc.surface.toy-elaborate-100` ×1.7). Recovery is ongoing.
