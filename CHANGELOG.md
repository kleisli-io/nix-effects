...
- **`ops/concerns` refinements internalize where the kernel can decide them.** String literal-enums → `oneOfStr`, `x ≥ 1` → `positiveInt`, and integer ranges (`Port` 1..65535, `MtaStsMaxAge` 60..31557600) → `inRangeInt`, so these fields' refinement types carry a non-null `ktype`. Predicates needing introspection the kernel lacks (`stringLength`, `builtins.match`, `lib.hasPrefix`, `lib.isDerivation`) stay raw lambdas.
