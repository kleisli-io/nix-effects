    ...
    expr =
      let
        ...
        result = handle
          ...
          comp;
      in
      (builtins.head result.state).context;
    expected = "validator";
  };
    ...
    expr =
      let
        ...
        result = handle
          ...
          comp;
      in
      builtins.all (e: e.passed) result.state;
    expected = true;
  };
    ...
    expr =
      let
        safeDivide = a: b:
          if b == 0
          then
            bind (conditions.signal "division-by-zero" { inherit a b; } [ "use-value" "return-zero" ])
              (response:
    ...
    expr =
      let
        ...
        result = handle
          ...
          comp;
      in
      { value = result.value; errors = result.state.errors; counter = result.state.counter; };
    expected = {
      value = 11; # 0 + 1 + 10
