  ...
  evalDescRef = env: ref:
    ref // {
      ...
      params = map (E.eval env) (ref.params or []);
          ...
          then { branchDescs = map (E.eval env) ref.branchDescs; }
          else {});
...
in {
  scope = {
    ...
    check = ctx: tm: ty:
      ...
      # Conv handles definitional irrelevance among inhabitants.
        ...
        bindP P.AnnType (self.checkType ctx tm.type) (aTm:
          ...
          bindP P.AnnTerm (self.check ctx tm.val aVal) (tTm:
            ...
            in bindP P.LetBody (self.check ctx' tm.body ty) (uTm:
          ...
          then send "typeError" {
            ...
            then send "typeError" {
              ...
              then
                ...
                bindP P.MuPayload (self.check ctx tm.d interpTy) (dataTm:
              ...
              in bindPChain [ (P.DConLayer n) P.MuIndex ] (
                ...
                in bindPChain [ (P.DConLayer n) P.MuPayload ] (self.check ctx base.d interpTyBase) (baseDataTm:
                    ...
                    let
                      ...
                      checkHeads = j: remaining: accTms:
                        if remaining == [] then pure accTms
                          ...
                          in bindPChain [ (P.DConLayer layerDepth) (P.Elem j) ]
      ...
      # through the CHECK→INFER bridge.
        ...
        bindP P.Sub (self.infer ctx tm) (result:
  ...
  __docs = {
    check = {
      description = "check: bidirectional checking-mode entry — verify `tm : ty` and return the elaborated kernel term; dispatches intro-form rules against their type-formers and falls through to synthesis + structural conv (the §7.4 sub rule).";
    ...
    checkMotive = {
      description = "checkMotive: eliminator-motive validation (§7.3) — walks the lambda layers of a motive against an expected domain chain `D_1 → … → D_n → U(k)` and returns the elaborated motive term together with the universe level `k` of its codomain.";
