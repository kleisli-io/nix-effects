...
let
  ...
  constructorSchema = c:
    let
      fields = c.fields or [ ];
      fieldProps = builtins.listToAttrs (map
        (f: {
          name = f.name;
          value = fieldSchema f;
        })
  ...
  tests = {
    ...
    "derive-descriptor-indexed-pi-field" = {
      expr =
        ...
        in
        (self.deriveDescriptor IndexedPi).constructors;
      expected = [
        {
          name = "mk";
          index = 0;
    ...
    "derive-descriptor-poly-datatype" = {
      expr =
        ...
        in
        (self.deriveDescriptor (H.app Tree.T H.nat)).constructors;
      expected = [
        {
          name = "leaf";
          index = 0;
    ...
    "derive-schema-poly-tree" = {
      expr =
        ...
        in
        self.deriveSchema (H.app Tree.T H.nat);
      expected = {
        title = "Tree";
        oneOf = [
          {
    ...
    "derive-descriptor-preserves-rich-field-metadata" = {
      expr =
        ...
        in
        (builtins.head (self.deriveDescriptor Indexed).constructors).fields;
      expected = [
        {
          name = "value";
          kind = "data";
