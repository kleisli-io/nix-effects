...
let
  ...
  mkEffectHeavy = n:
    ...
    in
    (handle { inherit handlers; state = 0; } (loop 0)).state;
  ...
  mkBindHeavy = n:
    ...
    in
    (handle { inherit handlers; state = null; } comp).value;
  ...
  mkNestedHandlers = depth: iterations:
    ...
    in
    (handle { handlers = { }; state = null; } wrapped).value;
  ...
  mkRawGC = n:
    ...
    in
    (builtins.elemAt steps (builtins.length steps - 1)).value;
