...
let
  ...
  rawOrnOf = ornamented:
    if isRawOrn ornamented then ornamented
  ...
  algShapeDiagnosticsOf = args:
    ...
    in algShapeDiagnosticsAt "root" descShape algShape;
  ...
  synthesisConstructorDiagnostics = args: con:
    ...
    in builtins.concatMap (synthesisFieldDiagnostics args con) fields;

  synthesisConstructorObligations = args: con:
    let
      conSpec = builtins.getAttr con.name args.spec.constructors;
      fields = conSpec.fields or [];
    in builtins.concatMap (synthesisFieldObligations args con) fields;
  ...
  synthesizeRecord = args: ornamented: i: baseValue:
    ...
    in self.review ornamented.T result.ornRecord;
...
in {
  scope = {
    inherit algSupportedFragment;

    ornament = base: spec:
      H.ornament base spec;

    forget = ornamented: value:
      forgetValue ornamented value;

    forgetHoas = forgetHoasOf;
  ...
  tests = {
    "ornaments-derive-descriptor-small" = {
      ...
      in (self.deriveDescriptor Tagged).constructors;
      expected = [
        {
          name = "box";
          index = 0;
    ...
    "ornaments-forget-small-checks" = {
      ...
      in !((H.checkHoas Box.T (self.forget Tagged value)) ? error);
      expected = true;
    };
    ...
    "ornaments-indexed-sugar-forget-checks" = {
      ...
      in !((H.checkHoas (HI.muI H.unit Base.D H.tt) forgot) ? error);
      expected = true;
    };
    ...
    "ornaments-pullback-small-checks" = {
      ...
      in !((H.checkHoas H.bool (self.pullback Tagged baseFn value)) ? error);
      expected = true;
    };
    ...
    "ornaments-pullbackHoas-small-checks" = {
      ...
      in !((H.checkHoas H.bool (H.app (H.app pulled H.tt) value)) ? error);
      expected = true;
    };
    ...
    "ornaments-liftFold-small-checks" = {
      ...
      in !((H.checkHoas H.bool (H.app (H.app lifted H.tt) value)) ? error);
      expected = true;
    };
    ...
    "ornaments-functional-build-small-checks" = {
      ...
      in !((H.checkHoas Box.T forgot) ? error);
      expected = true;
    };
    ...
    "ornaments-functional-liftProducer-canonicalizes-output" = {
      ...
      in self.view F.meta.ornamented.T built;
    ...
    "ornaments-functional-liftTransform-canonicalizes-output" = {
      ...
      in self.view Labeled.meta.ornamented.T built;
    ...
    "ornaments-algOrn-builder-summary-checks" = {
      ...
      in !((H.checkHoas forgetTy (self.forgetHoas O)) ? error);
      expected = true;
    };
    ...
    "ornaments-checkSpec-rejects-unknown-field" = {
      ...
      in (builtins.tryEval (self.checkSpec Box {
        name = "BadTaggedBox";
        constructors.box.fields = [
          { keep = "missing"; }
        ];
  ...
  __docs = {
    ...
    build = {
      description = "build: alias for `functionalBuild` — non-indexed section builder for a functional ornament. See `functionalBuild` for details.";
      signature = "build : FunctionalOrnament -> Tm -> Tm";
    };
    buildIndexed = {
      description = "buildIndexed: alias for `functionalBuildIndexed` — index-aware section builder for an indexed functional ornament. See `functionalBuildIndexed` for details.";
      signature = "buildIndexed : FunctionalOrnament -> Tm -> Tm -> Tm";
    };
    checkAlgShape = {
    ...
    section = {
      description = "section: alias for `functionalSection` — extract the `section` builder from a functional ornament. See `functionalSection` for details.";
      signature = "section : FunctionalOrnament -> (Tm -> Tm -> Tm)";
    };
    tryAlgOrn = {
