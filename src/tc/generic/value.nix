...
let
  ...
  mapChildValues = ty: childMapper: value:
    let
      ...
      mappedFields = builtins.listToAttrs (map (f: {
        name = f.name;
        value =
          if f.kind == "recAt"
    ...
    in self.view hoasTy (self.review hoasTy (record // mappedFields));
in {
  scope = {
    review = ty: value:
    ...
    view = ty: value:
      ...
      in Elab.extract hoasTy (E.eval [] (H.elab hoasValue));

    toConstructorRecord = self.view;

    fromConstructorRecord = self.review;

    fold = foldValue;

    mapChildren = mapChildValues;
  };
  ...
  tests = {
    ...
    "value-review-view-list" = {
      expr = let
        V = self;
        T = H.listOf H.nat;
      in V.view T (V.review T (V.view T [ 0 1 2 ]));
      expected = [ 0 1 2 ];
    };
    ...
    "value-review-view-sum" = {
      ...
      in V.view T (V.review T (V.view T value));
    ...
    "value-review-view-custom-tree" = {
      ...
      in V.view T (V.review T (V.view T record));
      expected = {
        _con = "node";
        left = { _con = "leaf"; value = 0; };
        right = { _con = "leaf"; value = 1; };
    ...
    "value-review-view-custom-mono-tree" = {
      ...
      in V.view Tree.T (V.review Tree.T (V.view Tree.T record));
      expected = {
        _con = "node";
        left = { _con = "leaf"; value = 0; };
        right = {
    ...
    "value-review-custom-tree-checks" = {
      ...
      in !((H.checkHoas T (V.review T record)) ? error);
      expected = true;
    };
    ...
    "value-review-rejects-unknown-constructor" = {
      ...
      in (builtins.tryEval (V.review Box.T { _con = "nope"; value = 0; })).success;
      expected = false;
    };
    ...
    "value-review-rejects-missing-field" = {
      ...
      in (builtins.tryEval (V.review Box.T { _con = "box"; })).success;
      expected = false;
    };
    ...
    "value-review-rejects-unknown-field" = {
      ...
      in (builtins.tryEval (V.review Box.T {
        _con = "box";
        value = 0;
        extra = true;
      })).success;
    ...
    "value-map-children-custom-tree-identity" = {
      ...
      in V.mapChildren T (_field: child: child) record;
      expected = {
        _con = "node";
        left = { _con = "leaf"; value = 0; };
        right = { _con = "leaf"; value = 1; };
