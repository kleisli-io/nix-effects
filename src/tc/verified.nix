...
let
  ...
  inl = leftTy: rightTy: term: HI.inlAtExplicit 0 leftTy rightTy term;
  # v.inr leftTy rightTy term — right injection
  inr = leftTy: rightTy: term: HI.inrAtExplicit 0 leftTy rightTy term;
  ...
  field = recordTy: fieldName: record:
    let
      ...
      matches = builtins.filter
        (i: (builtins.elemAt fields i).name == fieldName)
        idxs;
      idx =
        if matches == [ ]
      ...
      buildSelector = remaining: priorBinders:
        ...
        then builtins.elemAt priorBinders idx
          ...
          in
          H.lam f.name f.type (v: buildSelector rest (priorBinders ++ [ v ]));
  ...
  # `meta.constructors[0].ctor` (the HOAS constructor exposed by
  # `_datatypeImpl`).
...
in
api.namespace {
  description = "fx.tc.verified: high-level combinators for writing kernel-checked implementations; build a program, call `.verify` to type-check it against the kernel.";
  doc = ''
  ...
  tests =
    ...
    in
    {
      ...
      "v-matchList-nil" = {
        expr = verify NatT (matchList NatT NatT (H.nil) {
          nil = nat 0;
      ...
      "v-matchList-count" = {
        expr = verify NatT (matchList NatT NatT
          (H.cons (nat 1) (H.cons (nat 2) (H.cons (nat 3) (H.nil))))
          {
            nil = nat 0;
      ...
      "v-map-succ" = {
        expr =
          let
            ...
            input = H.cons (nat 0) (H.cons (nat 1) (H.cons (nat 2) (H.nil)));
            result = map NatT NatT succFn input;
          in
          verify (H.listOf NatT) result;
        expected = [ 1 2 3 ];
      };
      ...
      "v-map-empty" = {
        expr =
          let
            succFn = fn "x" NatT (x: H.succ x);
          in
          verify (H.listOf NatT) (map NatT NatT succFn (H.nil));
        expected = [ ];
      };
      ...
      "v-fold-sum" = {
        expr =
          let
            ...
            input = H.cons (nat 1) (H.cons (nat 1) (H.cons (nat 1) (H.nil)));
          in
          verify NatT (fold NatT NatT (nat 0) addFn input);
        expected = 3;
      };
      ...
      "v-filter-zeros" = {
        expr =
          let
            ...
            input = H.cons (nat 0) (H.cons (nat 1)
              (H.cons (nat 0) (H.cons (nat 2) (H.nil))));
          in
          verify (H.listOf NatT) (filter NatT isZero input);
        expected = [ 0 0 ];
      };
