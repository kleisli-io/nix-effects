...
- **Universe-polymorphic compound carriers.** `Record`, `ListOf`, `Maybe`, `Either`, and `Variant` now accept lifted children and build their carrier at the common universe `kH = max(child universes)` instead of throwing or pinning to `U(0)`. `ListOf` carries an explicit level parameter (`List : (k : Level) → U k → U k`, threaded through `listDesc` / `nil` / `cons` / `listElim`); the sum-desugared compounds (`Maybe` / `Either` / `Variant`) lift each branch carrier to `kH` via `LiftAt`; `Record` lifts each field to `kH` in its description encoder, so `Record { x = liftTo 2 Int }` lives at `U(2)`, reports `.universe = 2`, and is a member of `Type_2`. Every all-`U(0)` compound is byte-identical to before (`LiftAt u u = id`). The kernel structural predicate is sound only over all-`U(0)` branches, so higher-universe compounds fall back to the always-sound host guard.
- **`lift` / `liftTo` are exported on the surface** (alongside `typeAt` / `level`). 0.15.0 introduced the formers but did not expose them; now that lift dispatch decides correctly inside compounds they are public. Value-level `liftAt` / `lowerAt` stay internal.
- **Universe-level soundness corpus** (`kernel/level-soundness.nix`) — pins the term-independence invariant on three axes: site coverage, gate correctness, and decidability.

### Changed

- **Term-dependent universe levels are rejected uniformly at synthesis (breaking).** A synthesised `U(L)` is admitted only when `L` rests on `zero` / `suc` / `max` / bare-variable bases; an applied-neutral base (`f x`) makes the level term-indexed — conversion would hinge on term equality and the hierarchy would lose well-foundedness — so it is rejected at *every* site that reads a level to classify with it: the universe-forming rules in `infer`, the `checkTypeLevel` boundary, and the eliminator-motive codomains (non-lambda motives in `checkMotive` and the `J` motive check, which previously read the codomain level ungated — a level-polymorphic family applied at `f x` could otherwise type an eliminator at `U(suc (f x))`). The admissibility predicate is centralised in `conv` (`admitLevel`) as the single source of truth. This completes the partial gate shipped in 0.15.0, which left those motive paths ungated. Level polymorphism (`Π (k : Level). U(k)`) is unaffected.
- **Refinement and lift coherence — a type's universe label and proof obligations stay faithful to its kernel.** `refine` / `refined` propagate the base's declared universe instead of recomputing the kernel floor, so a refinement of a `U(n)` type stays at `U(n)` rather than collapsing to `U(0)`; `liftTo` now overrides `prove` alongside `_kernel` (the base proof is lifted via `liftAt` and checked against the lifted kernel, keeping `.prove` consistent with `._kernel`; approximate bases gain none); `typeAt`'s membership guard tightens `isType` to require the full `Type` interface (`check` / `validate` / `name`) rather than just the `_tag` — kernel-precise fields (`prove` / `kernelCheck`) are deliberately not required, so approximate `Pi` / `Sigma` remain valid universe members.
- **Term-dependent universe throws are attributed to their construction site.** A `universeOf ctx child` helper wraps each dependent constructor's child-universe read and re-throws with construction context (which constructor, which child) instead of the bare foundation error, staying lazy. Routes all six child-universe reads: `Pi.compose`, `Sigma.pullback` (fst and snd), `Certified` (decidable and propositional), and `DepRecord`.
- **Documentation clarifications.** `typeAt`'s `check` is the conjunction `decide(U(n),v) ∧ guard(v)` (the kernel verifies the level; the guard verifies type-ness and the exact universe label); `diagnose.kernel` reflects the kernel decision alone and is not authoritative membership; `level` documents that it throws on a term-dependent or level-polymorphic type; `levelOf`'s level-polymorphic throw names the cause; `liftTo`'s down-lift error includes the type name.
- **`matching` documented as a deliberately raw host predicate.** `matching` (regex via `builtins.match`) is not kernel-internalized — its `.ktype` is null, so it is boundary-checked and does not fold into a `KernelPred`, and a compound containing it is not kernel-decided. A literal-pattern variant that internalizes the match is feasible but deferred until a concrete use case justifies it and its normalization cost is validated.

### Fixed

- **Lifted compounds are dispatched in the generic structural walker.** `deriveGo` had no arm for the `lift` kernel tag, so a compound carrying a lifted child (e.g. `Record { x = liftTo 2 Int }`) fell through to the unhandled-shape path and failed to check or elaborate. A `walkLift` arm now descends into `ty.A` at the same value and path (a lift shares its carrier's inhabitants; the coercion is type-level only); the elaborate fold re-wraps via `liftAt` / `liftAtWithEq` so the term stays typed at `LiftAt l m A`. Fixes lifted-field `Record` and single-branch `Variant`.
- **Metavariable levels are tolerated in elaboration-time conversion.** `closedLevelNat` and `flattenLevel` read `.tag` unguarded and crashed when a level summand is a `VMeta` (an unsolved hole, no `.tag`); a meta level is now treated as an opaque base summand (ordered after rigid variables by id), so conversion builds a stuck `VLift` instead of crashing. The kernel never holds metas, so the new branches are dead on kernel inputs and conversion is byte-identical there.
- **The universe-soundness gate fails closed.** Removed a dead, fail-*open* `or [ ]` default from `admitLevel`'s var-spine check: every var base carries a spine by construction, so the default never fired, but had a malformed spineless base reached it the guard would have *admitted* an unverifiable level. The single predicate behind every universe-soundness gate now relies on its invariant and throws on violation rather than silently admitting.
...
- **`certifyProofE` — effectful dual of `certifyProof`.** `Certified`'s general (propositional-family) former gains an effectful introduction matching the decidable former's `certifyE`: pure on a valid proof, and emitting a `typeCheck` effect (rather than throwing) on a non-checking proof or a base value that fails its type.
- **`mkPropFamily { family; bridge; }` — a bundled handle for `Certified`'s general former.** Packages a propositional family and its bridge into one value, passed via the `family` argument; the two-argument `{ family; bridge; }` form remains valid.
- **`lift` / `liftTo` universe coercions.** `liftTo m t` reindexes a type to universe `m` (require `m >= t.universe`), preserving its values (`check` is unchanged) and exposing the kernel `LiftAt` of its kernel type; `lift t = liftTo (t.universe + 1) t`. The explicit cross-level coercion the now non-cumulative `Type_n` tower requires; idempotent at `m == t.universe`.

### Changed

- **Universe levels are total and cannot depend on a term (soundness, breaking).** `.universe` no longer fabricates a level. A type's computed universe is a total, exact `Int`, and reading `.universe` on a non-type, on a level-polymorphic type, or on a type whose level would depend on a term now throws instead of silently returning `0`. The kernel rejects term-dependent universe levels at type formation: `U(ℓ)` is admitted only when `ℓ` lies in the `zero | suc | max | variable` fragment, so an applied-neutral level — the `sup_{a:A} level(B(a))` tail whose codomain level depends on the domain value — is a `typeError` rather than a trusted `0`. A declared `universe = N` is still enforced against the kernel minimum (under-declaring throws at construction, over-declaring a higher level stays sound), but the declaration can no longer rescue a type that has no concrete level — it throws. (Supersedes the partial decidable-fragment enforcement from earlier in this cycle, which double-guarded the level computation and trusted the undecidable tail.)
- **`Type_n` is a non-cumulative Tarski tower (breaking).** `Type_n` now contains exactly the types of universe `n` (membership is `==`, not `<=`): a lower-level type is no longer implicitly subsumed into a higher universe. Use the new `lift` / `liftTo` to move a type up the tower explicitly. The surface hierarchy now mirrors the kernel, which is Tarski-style and non-cumulative.
...
- **A decidable `Certified` nested inside a compound is no longer over-rejected.** When a decidable `Certified` (a kernel-predicate witness, `_kernel = El t`) appeared as a `Record` field, `ListOf` element, `Variant` arm, or `Sigma` component, the monolithic kernel `.check` rejected every value — valid ones included — and diverged from the structural `.validate`, which accepted them. The cause: `El t` is `app`-headed and carries no datatype metadata, so the generic structural walker reached it as a dead end and failed closed. The walker now normalizes a metadata-less `app`-headed kernel type before dispatch — `El t` WHNF-reduces to its `Σ` form, whose carrier is walked structurally and whose predicate is decided by instantiating the kernel closure at the concrete value (`Unit` ⇒ accept, `Empty` ⇒ reject, anything else ⇒ fail closed). `.check` now agrees with `.validate` for the nested form at every depth. The `El` WHNF and its lowered term are cached once per `Certified` type, so checking a collection pays the `El` elaboration once rather than per value. Membership for the nested form normalizes the predicate in the kernel and therefore sits in the same cost class as standalone kernel-decided membership; where a kernel witness is not required, `.validate` (host-side structural verify) decides the same accept/reject far more cheaply.
