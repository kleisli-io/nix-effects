...
in {
  ...
  __docs = {
    _self = {
      description = "Refinement types and predicate combinators: `refined` plus `allOf`/`anyOf`/`negate` and ready-made predicates `positive`/`nonNegative`/`inRange`/`nonEmpty`/`matching`.";
      doc = ''
